...
export default async function handler(req, res) {
  ...
  const partial = sorted.filter(([d, count]) => count < total && count > 1).slice(0, 12).map(([date, count]) => ({
    date, count,
    names: dateNames[date],
    missing: allNames.filter(n => !dateNames[date].includes(n))
  }));
