...
const getHeaders = () => ({ Authorization: `Bearer ${process.env.KV_REST_API_TOKEN}` });
...
export default async function handler(req, res) {
  try {
    if (req.method === 'POST') {
      ...
      return res.status(200).json({ code, name, verify, memberCount: Object.keys(verify?.members || {}).length });
    }
    if (req.method === 'GET') {
      const { code } = req.query;
      const group = await kvGet(`group:${code}`);
      if (!group) return res.status(404).json({ error: 'Not found' });
      return res.status(200).json({ name: group.name,
